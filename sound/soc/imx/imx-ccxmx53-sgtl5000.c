/*
 * imx-ccxmx53-sgtl5000.c  --  i.MX CCXMX53 Driver for Freescale SGTL5000 Codec
 *
 ...
 *
 * Based on soc/imx/imx-3stack-sgtl5000.c
 *
 */
...
struct imx_ccxmx53_priv {
	int sysclk;
	int hw;
	struct platform_device *pdev;
};

static struct imx_ccxmx53_priv card_priv;

static int imx_ccxmx53_audio_hw_params(struct snd_pcm_substream *substream,
				      struct snd_pcm_hw_params *params)
{
	...
	struct imx_ccxmx53_priv *priv = &card_priv;
...
static int imx_ccxmx53_startup(struct snd_pcm_substream *substream)
{
#if defined(CONFIG_MXC_ASRC) || defined(CONFIG_MXC_ASRC_MODULE)
...
static void imx_ccxmx53_shutdown(struct snd_pcm_substream *substream)
{
	struct imx_ccxmx53_priv *priv = &card_priv;
...
/*
 * imx_ccxmx53 SGTL5000 audio DAI opserations.
 */
static struct snd_soc_ops imx_ccxmx53_ops = {
	.startup = imx_ccxmx53_startup,
	.shutdown = imx_ccxmx53_shutdown,
	.hw_params = imx_ccxmx53_audio_hw_params,
};

static void imx_ccxmx53_init_dam(int ssi_port, int dai_port)
{
...
/* imx_ccxmx53 machine connections to the codec pins */
...
/* imx_ccxmx53 card dapm widgets */
static const struct snd_soc_dapm_widget imx_ccxmx53_dapm_widgets[] = {
...
static int imx_ccxmx53_sgtl5000_init(struct snd_soc_codec *codec)
{
	int i, ret;
	...
	/* Add imx_ccxmx53 specific controls */
	...
	/* Add imx_ccxmx53 specific widgets */
	snd_soc_dapm_new_controls(codec, imx_ccxmx53_dapm_widgets,
				  ARRAY_SIZE(imx_ccxmx53_dapm_widgets));

	/* Set up imx_ccxmx53 specific audio path audio_map */
...
/* imx_ccxmx53 digital audio interface glue - connects codec <--> CPU */
static struct snd_soc_dai_link imx_ccxmx53_dai = {
	.name = "SGTL5000",
	.stream_name = "SGTL5000",
	.codec_dai = &sgtl5000_dai,
	.init = imx_ccxmx53_sgtl5000_init,
	.ops = &imx_ccxmx53_ops,
};

static int imx_ccxmx53_card_remove(struct platform_device *pdev)
{
	struct imx_ccxmx53_priv *priv = &card_priv;
	struct mxc_audio_platform_data *plat;
	if (priv->pdev) {
...
static struct snd_soc_card snd_soc_card_imx_ccxmx53 = {
	.name = "imx-ccxmx53",
	.platform = &imx_soc_platform,
	.dai_link = &imx_ccxmx53_dai,
	.num_links = 1,
	.remove = imx_ccxmx53_card_remove,
};

static struct snd_soc_device imx_ccxmx53_snd_devdata = {
	.card = &snd_soc_card_imx_ccxmx53,
	.codec_dev = &soc_codec_dev_sgtl5000,
};

static int __devinit imx_ccxmx53_sgtl5000_probe(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_ccxmx53_priv *priv = &card_priv;
	...
	imx_ccxmx53_init_dam(plat->src_port, plat->ext_port);
	...
	if (plat->src_port == 2){
		...
		pr_err("Invalid source port %d\n",plat->src_port);
		goto port_err;
	}

	imx_ccxmx53_dai.cpu_dai = sgtl5000_cpu_dai;
	...
	imx_ccxmx53_snd_devdata.codec_data = setup;
...
static int imx_ccxmx53_sgtl5000_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_ccxmx53_priv *priv = &card_priv;
...
static struct platform_driver imx_ccxmx53_sgtl5000_audio_driver = {
	.probe = imx_ccxmx53_sgtl5000_probe,
	.remove = imx_ccxmx53_sgtl5000_remove,
	.driver = {
		   .name = "imx-ccxmx53-sgtl5000",
		   },
};

static struct platform_device *imx_ccxmx53_snd_device;

static int __init imx_ccxmx53_init(void)
{
	int ret;
	ret = platform_driver_register(&imx_ccxmx53_sgtl5000_audio_driver);
	if (ret)
		return -ENOMEM;

	imx_ccxmx53_snd_device = platform_device_alloc("soc-audio", 2);
	if (!imx_ccxmx53_snd_device)
		return -ENOMEM;

	platform_set_drvdata(imx_ccxmx53_snd_device, &imx_ccxmx53_snd_devdata);
	imx_ccxmx53_snd_devdata.dev = &imx_ccxmx53_snd_device->dev;
	ret = platform_device_add(imx_ccxmx53_snd_device);

	if (ret)
		platform_device_put(imx_ccxmx53_snd_device);

	return ret;
}

static void __exit imx_ccxmx53_exit(void)
{
	platform_driver_unregister(&imx_ccxmx53_sgtl5000_audio_driver);
	platform_device_unregister(imx_ccxmx53_snd_device);
}

module_init(imx_ccxmx53_init);
module_exit(imx_ccxmx53_exit);

MODULE_AUTHOR("Digi International.");
MODULE_DESCRIPTION("SGTL5000 Driver for CCXMX53.");
MODULE_LICENSE("GPL");
