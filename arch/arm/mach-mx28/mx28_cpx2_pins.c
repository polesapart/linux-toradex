...
static struct pin_desc mx28_cpx2_eth_pins[] = {
	{
	 .name = "ENET0_MDC",
	 .id = PINID_ENET0_MDC,
	 .fun = PIN_FUN1,
...
static struct pin_desc mx28_cpx2_ssp1_pins[] = {
	{
	 .name	= "SSP1_DATA0",
	 .id	= PINID_GPMI_D00,
	 .fun	= PIN_FUN2,
...
static struct pin_desc mx28_cpx2_gpmi_pins[] = {
	{
	 .name     = "GPMI D0",
	 .id       = PINID_GPMI_D00,
	 .fun      = PIN_FUN1,
...
static struct pin_desc mx28_cpx2_spi_pins[] = {
	{
	 .name	= "SSP2 MOSI",
	 .id	= PINID_SSP2_MOSI,
	 .fun	= PIN_FUN1,
...
#if defined(CONFIG_FEC) || defined(CONFIG_FEC_MODULE)\
	|| defined(CONFIG_FEC_L2SWITCH)
int mx28_cpx2_enet_gpio_init(void)
{
	/* pwr */
...
void mx28_cpx2_enet_io_lowerpower_enter(void)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(mx28_cpx2_eth_pins); i++) {
		mxs_release_pin(mx28_cpx2_eth_pins[i].id,
			mx28_cpx2_eth_pins[i].name);
		gpio_request(MXS_PIN_TO_GPIO(mx28_cpx2_eth_pins[i].id),
			mx28_cpx2_eth_pins[i].name);
		gpio_direction_output(
			MXS_PIN_TO_GPIO(mx28_cpx2_eth_pins[i].id), 0);
	}

}

void mx28_cpx2_enet_io_lowerpower_exit(void)
{
	int i;
	gpio_direction_output(MXS_PIN_TO_GPIO(PINID_SSP1_DATA3), 0);
	gpio_direction_output(MXS_PIN_TO_GPIO(PINID_ENET0_RX_CLK), 1);
	...
	for (i = 0; i < ARRAY_SIZE(mx28_cpx2_eth_pins); i++) {
		gpio_free(MXS_PIN_TO_GPIO(mx28_cpx2_eth_pins[i].id));
		mxs_request_pin(mx28_cpx2_eth_pins[i].id,
			mx28_cpx2_eth_pins[i].fun,
			mx28_cpx2_eth_pins[i].name);
	}
}
