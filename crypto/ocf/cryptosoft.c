...
//	{ cbc(aes),          CRYPTO_TFM_MODE_CBC, SW_TYPE_BLKCIPHER },
	{ cbc(aes),          CRYPTO_TFM_MODE_CBC, SW_TYPE_ABLKCIPHER },
...
static void ablkcipher_complete(struct crypto_async_request *req, int err);
...
/*
 * Completion function for ablkciphers
 */
static void ablkcipher_complete(struct crypto_async_request *req, int err)
{
	struct ablkcipher_result *res = req->data;

	if (err == -EINPROGRESS)
		return;

	printk("%s(): Calling req->completion\n", __FUNCTION__);
	res->err = err;
	...
	*sid = i;
	...
	while (cri) {
		*swd = (struct swcr_data *) kmalloc(sizeof(struct swcr_data),
		...
		if (sw_type == SW_TYPE_BLKCIPHER) {
		...
		} else if (sw_type == SW_TYPE_ABLKCIPHER) {
			dprintk("%s crypto_alloc_ablkcipher(%s, 0x%x)\n", __FUNCTION__,
					algo, mode);

			(*swd)->sw_tfm = crypto_ablkcipher_tfm(
								crypto_alloc_ablkcipher(algo, 0,
									0));
			if (!(*swd)->sw_tfm) {
				dprintk("cryptosoft: crypto_alloc_ablkcipher failed(%s,0x%x)\n",
						algo,mode);
				swcr_freesession(NULL, i);
				return EINVAL;
			}
			...
			error = crypto_ablkcipher_setkey(
						__crypto_ablkcipher_cast((*swd)->sw_tfm), cri->cri_key,
							(cri->cri_klen + 7) / 8);
			if (error) {
				printk("cryptosoft: setkey failed %d (crt_flags=0x%x)\n", error,
						(*swd)->sw_tfm->crt_flags);
...
#define SCATTERLIST_MAX 16
	...
	for (crd = crp->crp_desc; crd; crd = crd->crd_next) {
		...
		case SW_TYPE_ABLKCIPHER: {
			...
			struct ablkcipher_result result;
			unsigned int ret;

			init_completion(&result.completion);
			req = ablkcipher_request_alloc( __crypto_ablkcipher_cast(sw->sw_tfm), GFP_KERNEL);
			if (!req) {
				dprintk("failed to allocate request\n");
			...
			if (1) { /* Flag not implemented for cryptosoft */
			/*if (crd->crd_flags & CRD_F_KEY_EXPLICIT) {*/
				int i, error;
			...
			/* Fill in ablkcipher_request struct */
			ablkcipher_request_set_callback(req, CRYPTO_TFM_REQ_MAY_BACKLOG,
					ablkcipher_complete, &result);
...
//				}
				/* Fill in ablkcipher_request struct */
				ablkcipher_request_set_crypt(req, sg, sg, sg_len, ivp);
	...
	for (i = CRYPTO_ALGORITHM_MIN; i <= CRYPTO_ALGORITHM_MAX; ++i)
	{
		...
		switch (sw_type)
		{
			...
			case SW_TYPE_ABLKCIPHER:
				...
				{
					dprintk("%s:ABLKCIPHER algorithm %d:'%s' not supported\n",
								__FUNCTION__, i, algo);
				}
				break;
			default:
