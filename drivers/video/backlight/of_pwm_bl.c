	...
	*of_pwm_bl_parse_platform_data(struct platform_device *ndev)
{
	...
	if (!of_property_read_u32(np, "nvidia,disable-edp-throttle", &temp)) {
		pdata->disable_edp_throttle = (bool)temp;
		OF_PWMBL_LOG("disable edp throttle %d\n",
			pdata->disable_edp_throttle);
	}
